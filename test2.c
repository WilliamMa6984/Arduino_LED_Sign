...
volatile int mtrxPoints[MAX_MTRX_POINTS][2] = {-1};
...
void turnOnLEDs()
{
	...
	for (int index = 0; index <= numPoints[index]; index++)
	{
		...
		blinkLED(mtrxPoints[index][0], mtrxPoints[index][1]);
	}
}
	...
	switch (byte)
	{
		...
		case '1': // LED point - on
				...
				row >= rowOffset && row <= rowOffset + rowSpan)
			{
				...
				uart_putbyte('\n');
				// Found point
				mtrxPoints[mtrxPointIndex][0] = rowPins[row-rowOffset];
				mtrxPoints[mtrxPointIndex][1] = colPins[col-colOffset];
			}
...
// -----------------------------------------------------------------------TESTING: writes to console what was received
void uart_putbyte(unsigned char data) {
